...
**The Quiet Enjoyment Project · September 2026 · Version 1.0**

This package contains every dataset, script, and figure behind the report
*Understanding Noise Pollution in Boulder* (QEP, September 2026), published per
the report's transparency commitment. Every number in the report can be
re-derived from these files. Questions and corrections:
yourneighbor@quietenjoymentproject.org — we publish corrections promptly and
prominently if we err.

`checksums.txt` carries a SHA-256 hash of every file so that any copy of this
package can be verified against the published original.
...
| `cu_walkshed_15min.kmz` | The CU walkshed: 15-minute (0.75-mile) pedestrian buffer from the university-district boundary | QEP, from the City's official subcommunity polygon |
...
| `health_exposure_results.json` | Resident-night exposure statistics (211,483/yr; 45% school nights) | Derived; see `health_exposure.py` |
...
| 10.4 complaints/night Thu–Sat in term; one per 46 min; Sept weekends 19; worst night 38 (Oct 30, 2025) | noiseb + walkshed | `night_stats.py` |
| Term 5.6/night (73%) vs winter break 1.1; move-in 10.2 (78%) | noiseb + walkshed | `term_break_analysis.py` |
| School-year trend 1,408 → 1,515 → 1,777; spring 2026 +30% | noiseb + walkshed | `report_charts2.py`, `report_maps_v3.py` |
| 3.5x complaints per rental inside the walkshed (0.26 vs 0.07) | rental calls + rental parcels + walkshed | `report_charts2.py` |
| 28,010 permanent residents (26% of city), incl. 3,957 children; 4.6x per-person burden | census blocks + walkshed | `permanent_pop2.py`, `age_split.py`, `report_stats.py` |
| 55% of complaints 10 p.m.–3 a.m.; hourly distribution | noiseb | `report_charts2.py` |
| 211,000 resident-nights of sleep disruption; 45% school nights | party/noise calls + parcels | `health_exposure.py` |
